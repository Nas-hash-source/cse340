...
async function getMessages(account_id) {
  try {
    const sql = "SELECT m.message_id, message_created, m.message_subject, m.message_read, CONCAT(a.account_firstname, ' ', a.account_lastname) AS full_name FROM public.message m INNER JOIN public.account a ON m.message_to = a.account_id WHERE m.message_to = $1 AND m.message_archived = FALSE ORDER BY m.message_CREATED DESC"
...
async function getMessageById(message_id) {
  try {
    const sql = "SELECT m.message_subject, m.message_body,a.account_id, CONCAT(a.account_firstname, ' ', a.account_lastname) AS full_name FROM public.message m INNER JOIN public.account a ON m.message_from = a.account_id WHERE m.message_id = $1"
...
async function getArchivedMessages(account_id) {
  try {
    const sql = "SELECT m.message_id, m.message_subject, m.message_read, CONCAT(a.account_firstname, ' ', a.account_lastname) AS full_name FROM public.message m INNER JOIN public.account a ON m.message_to = a.account_id WHERE m.message_to = $1 AND m.message_archived = TRUE ORDER BY m.message_CREATED DESC"
...
async function remmoveMessage(message_id) {
  try {
